...
export default class SlideVerify {
    constructor({elementId, onSuccess, onFail, onRefresh, onVerify, lang, photo, position, extraInfo, fixed, showClose, onClose}) {
        let intlText = {}
        if (lang && lang === 'en') {
        ...
        let closeIcon = childNodes[2]
        if (showClose) {
            closeIcon.innerHTML = Icons.close
        }
        let block = childNodes[3]
        let sliderContainer = childNodes[4]
    ...
    clickClose() {
        typeof this.onClose === 'function' && this.onClose({extra: this.extraInfo})
        this.destory()
    }
    ...
    bindEvents() {
        ...
        this.closeIcon.onclick = () => this.clickClose()
